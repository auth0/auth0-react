...
// / Type Aliases / Variables" with every export flat inside. It ignores the
...
// show the curated nav and the flat kind-based nav together.
//
...
const ARTIFACT = 'mintlify/docsv2/sdk-artifacts/auth0-react.json';
const TARGET =
  'mintlify/docsv2/config/navigation/generated/react-reference.en.json';
 ...
 */
const DIRECTORY = 'docs/sdk/react';
...
const TOP_CATEGORIES = ['Getting Started', 'Hooks & HOCs', 'Context'];
...
// The local HTML sidebar hangs the context interface's members off its node.
// That is deliberately not mirrored here: Mintlify has no way to label a nav
// entry (there is no `label` or `title` field on a page), so an anchor entry
// renders with its raw slug, giving rows reading
// "Auth0ContextInterface#getaccesstokensilently". Mintlify already lists every
// member in the right-hand table of contents on the interface page, which is the
// same one-click access without the sidebar noise.
